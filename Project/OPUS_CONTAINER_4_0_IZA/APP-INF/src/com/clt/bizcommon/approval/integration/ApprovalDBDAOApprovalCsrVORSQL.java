...
public class ApprovalDBDAOApprovalCsrVORSQL implements ISQLTemplate{
	...
	public void setQuery(){
		...
		query.append(", T.USD_APRO_TTL_AMT	" ).append("\n"); 
		...
		query.append("    , X.PAY_DUE_DT, X.CURR_CD, X.APRO_TTL_AMT, ROUND(X.APRO_TTL_AMT/USD_LOCL_XCH_RT,C.DP_PRCS_KNT) USD_APRO_TTL_AMT" ).append("\n"); 
		...
		query.append("	         , TRIM(TO_CHAR(B.VNDR_SEQ, '000000')) VNDR_SEQ" ).append("\n"); 
		query.append("	         , B.PAY_DUE_DT          	 " ).append("\n"); 
		query.append("	         , B.CURR_CD          	 " ).append("\n"); 
		query.append("	         , B.APRO_TTL_AMT          	 " ).append("\n"); 
		query.append("             , C.APRO_RMK APRO_RMK" ).append("\n"); 
		...
		query.append("    #end" ).append("\n"); 
		query.append("    AND X.RQST_OFC_CD LIKE '%'||@[ofc_cd_deptsrch]||'%'" ).append("\n"); 
		query.append("    AND X.APRO_USR_ID = U.USR_ID " ).append("\n"); 
		query.append("    AND NVL(U.USE_FLG,'N') = 'Y'" ).append("\n"); 
		query.append("    AND X.APRO_USR_ID = @[usr_id]" ).append("\n"); 
		query.append("    AND R.ACCT_XCH_RT_LVL(+) = '1'" ).append("\n"); 
		query.append("    AND SUBSTR(D.GL_DT,1,6) = R.ACCT_XCH_RT_YRMON(+)" ).append("\n"); 
		query.append("    AND X.CURR_CD = R.CURR_CD(+)" ).append("\n"); 
		query.append("    AND X.CURR_CD = C.CURR_CD(+)" ).append("\n"); 
