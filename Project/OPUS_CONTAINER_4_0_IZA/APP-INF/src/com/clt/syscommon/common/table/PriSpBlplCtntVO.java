...
public class PriSpBlplCtntVO extends AbstractValueObject {
	...
	public void fromRequest(HttpServletRequest request) throws Exception {
		...
		try{
			String blplCnt = JSPUtil.getParameter(request, "blpl_ctnt", "");
			setBlplCtnt(URLDecoder.decode(blplCnt,"UTF-8"));
		} catch(UnsupportedEncodingException e){
			log.debug("\n[USER ERROR UnsupportedEncodingException] ["+this.getClass().getName()+".perform(blpl_ctnt)] "+e.getMessage());
			throw new Exception(e.getMessage());
		} catch(Exception e){
			log.debug("\n[USER ERROR Exception] ["+this.getClass().getName()+".perform(blpl_ctnt)] "+e.getMessage());
			throw new Exception(e.getMessage());
		}
