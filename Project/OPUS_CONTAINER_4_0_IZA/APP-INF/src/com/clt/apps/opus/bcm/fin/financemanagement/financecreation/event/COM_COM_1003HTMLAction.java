...
*@FileName : COM_COM_1003HTMLAction.java
 ...
 * @see ComCom1003Event 참조
 * @since J2EE 1.6
 */
public class COM_COM_1003HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 ComCom1003Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		ComCom1003Event event = new ComCom1003Event();
		if (command.isCommand(FormCommand.SEARCH) || command.isCommand(FormCommand.COMMAND01) || command.isCommand(FormCommand.REPLY)) {
			event.setGlEstmRevVvdVO((GlEstmRevVvdVO)getVO(request, GlEstmRevVvdVO.class));
		} else if (command.isCommand(FormCommand.MULTI)) {
			event.setGlEstmRevVvdVOs((GlEstmRevVvdVO[]) getVOs(request, GlEstmRevVvdVO.class, "sheet1_"));
		}
		return event;
	}
