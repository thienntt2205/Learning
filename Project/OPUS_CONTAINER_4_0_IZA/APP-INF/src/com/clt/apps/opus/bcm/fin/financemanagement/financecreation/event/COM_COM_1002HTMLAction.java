...
*@FileName : COM_COM_1002HTMLAction.java
 ...
 * @see ComCom1002Event 참조
 * @since J2EE 1.6
 */
public class COM_COM_1002HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 ComCom1002Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {	
		...
		ComCom1002Event event = new ComCom1002Event();
		if (command.isCommand(FormCommand.SEARCH)) {
			event.setGlErrVvdVO((GlErrVvdVO)getVO(request, GlErrVvdVO.class));
		} else if (command.isCommand(FormCommand.MULTI)) {
			event.setGlErrVvdVOs((GlErrVvdVO[]) getVOs(request, GlErrVvdVO.class, "sheet1_"));
		}
		return event;
	}
