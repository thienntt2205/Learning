...
*@FileName : COM_COM_1004HTMLAction.java
 ...
 * @see ComCom1004Event 참조
 * @since J2EE 1.6
 */
public class COM_COM_1004HTMLAction extends HTMLActionSupport {

	private static final long serialVersionUID = 1L;

	/**
	 * HTML DOM 객체의 Value를 자바 변수로 Parsing<br>
	 * HttpRequst의 정보를 ComCom1004Event로 파싱하여 request에 셋팅<br>
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
		...
		ComCom1004Event event = new ComCom1004Event();
		if (command.isCommand(FormCommand.SEARCH)) {
			event.setGlFmcEntVO((GlFmcEntVO)getVO(request, GlFmcEntVO.class));
		} else if (command.isCommand(FormCommand.MULTI)) {
			event.setGlFmcEntVOs((GlFmcEntVO[]) getVOs(request, GlFmcEntVO.class, "sheet1_"));
		}
		return event;
	}
