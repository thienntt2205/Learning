...
*@FileName : BCM_SUP_0001HTMLAction.java
*@FileTitle : EDI CROUP
*Open Issues :
*Change history :
 ...
 * @see BcmSup0001Event 참조
 * @since J2EE 1.6
 */

public class BCM_SUP_0001HTMLAction extends HTMLActionSupport {
	
	private static final long serialVersionUID = 1L;

	public BCM_SUP_0001HTMLAction() {}
	...
	public Event perform(HttpServletRequest request) throws HTMLActionException {
    	...
    	BcmSup0001Event event = new BcmSup0001Event();
		
		if(command.isCommand(FormCommand.MULTI)) {
			event.setSearchGroupListVOS((SearchGroupListVO[])getVOs(request, SearchGroupListVO.class,""));

			event.setGrpCd(request.getParameter("s_grp_cd"));
			event.setCoDivCd(request.getParameter("s_co_div_cd"));
			event.setEdiGrpDesc(request.getParameter("s_edi_grp_desc"));
			event.setCustTrdPrnrId(request.getParameter("s_cust_trd_prnr_id"));
