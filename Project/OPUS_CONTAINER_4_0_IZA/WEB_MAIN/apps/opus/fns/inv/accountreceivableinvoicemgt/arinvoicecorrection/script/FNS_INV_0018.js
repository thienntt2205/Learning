...
//			      SetConfig( { SearchMode:2, MergeSheet:0, Page:20, DataRowMerge:1 } );
			      ...
			      var cols = [ {Type:"Status",    Hidden:1, Width:40,   Align:"Center",  ColMerge:1,   SaveName:"ibflag",           KeyField:0,   CalcLogic:"",   Format:"" },
			             ...
			             {Type:"Text",      Hidden:1,  Width:50,   Align:"Center",  ColMerge:1,   SaveName:"ida_oft_expt_flg", KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 }];
...
//			      sheetObj.ShowSubSum([{StdCol:"curr_cd", SumCols:"7", Sort:false, ShowCumulate:true, CaptionCol:0, CaptionText:"Total"}]);
	...
	//							sheetObj.ShowSubSum([{StdCol:"curr_cd", SumCols:"chg_amt", Sort:0,   CaptionCol:2, CaptionText:"Total"}]);
				...
				if (ComShowCodeConfirm("INV00160")) {   
					...
					*/
					...
					if (backendJobKey != undefined && backendJobKey != '') {
						...
						// 2018.11.02 jkyang :: 중복생성 체크 
						var savFlg=ComGetEtcData(sXml, "inv_sav_flg");
						if(savFlg=="P") {
		...
		case IBSEARCH_ASYNC20: // Tax country retrieve
			...
			break;
		case MODIFY: // 중복저장 방지. INV_SAV_FLG = 'N' update. 2018.11.02 jkyang
...
//						InsertItem( document.form.ofc.value.substring(0,"0")+document.form.ofc.value.substring(0,3) + ComLpad(min_seq));
	...
	function sheet2_OnSearchEnd(sheetObj, ErrMsg){
		...
		for(k=1;k<=sheetObj.RowCount();k++){
			...
			if(office_cnt_cd == "IN"){
			   if (parseFloat(sheetObj.GetCellValue(k, "inv_xch_rt"))== 0 ){
				    no_ex = no_ex + 1;
			   }
			}
		}	
		if(office_cnt_cd == "IN" && parseInt(no_ex) > 0){
            ComBtnDisable("btn_go");
			ComBtnDisable("btn_paper");
			ComBtnDisable("btn_save");
            ComShowCodeMessage("INV00177");
		}
...
//		sheetObj.ShowSubSum([{StdCol:"curr_cd", SumCols:"7", Sort:0,   CaptionCol:2, CaptionText:"Total"}]);
	...
	function ar_ofc_cd_OnChange(comboObj,oldIndex, oldText, oldCode, newIndex, newText, newCode){ 
	    ...
	    formObj.office_cnt_cd.value=arrStr[6].substring(0,2); //India GST 사용 목적 
...
//JHJ 2018-07-09 start!
	...
	function fn_cust_gst_no(){
		...
		if ((document.form.cust_cnt_cd.value.trim() != "" )&&(document.form.cust_seq.value.trim() != "")&&(document.form.cust_seq.value.trim() != "000000")){
			...
			var sXml=sheetObjects[0].GetSearchData("FNS_INV_0018GS.do", FormQueryString(document.form));
...
//*********** JHJ 2018.01  END!  *************      
	...
	function getBackEndJobStatusSAVE() {
		...
		if(jobState == "3") {
			...
			clearInterval(timer);
	 		...
	 		ComOpenWait(false);
			doActionIBSheet(sheetObj, formObj, MODIFY);	// 2018.11.02 jkyang..중복저장 방지
		} else if(jobState == "5") {
			clearInterval(timer);
			ComShowCodeMessage("INV00090");
			ComOpenWait(false);
	...
	function checkRepCustomer(sheetObj){
		var formObject=document.form;	 
		document.form.f_cmd.value=SEARCH04;

		var sXml=sheetObj.GetSearchData("FNS_INV_0018GS.do", FormQueryString(formObject));
	...
	function fn_inv_cust_chg(){
		if(form.frm_act_cust_seq.value!=''){
			form.frm_act_cust_seq.value=ComLpad(form.frm_act_cust_seq.value.trim(), 6, "0");
		}
		form.cust_cnt_cd.value=form.frm_act_cust_cnt_cd.value;
		form.cust_seq.value=form.frm_act_cust_seq.value;
