...
function initSheet(sheetObj, sheetNo) {
    ...
    switch (sheetObj.id) {
        case "sheet1": // t1sheet1 init     //t1sheet1 init
            with(sheetObj){
                var HeadTitle1 ="|Del|Chk|Dummy\nFlag|VVD or Vessel\n(for dummy)|Activity\nDate|Lane|Trade|Partner|Curr.|Rev/Exp|Item|Amount|Remark|Invoice No|CSR No|Accrued";
                    HeadTitle1 +="|vsl_cd|skd_voy_no|skd_dir_cd|rev_dir_cd|rev_yrmon|stl_vvd_seq|st_dt|end_dt|acct_cd|auth_ofc_cd|auth_flg|chk_del_flg|valid_col_cd|valid_cd|valid_nm|valid_msg|valid_row";
                ...
                var cols = [ 
                       ...
                       {Type:"Float",     Hidden:0, Width:120,  Align:"Right",   ColMerge:0,   SaveName:"act_amt",          KeyField:0,   CalcLogic:"",   Format:"Float",       PointCount:2,   UpdateEdit:1,   InsertEdit:1 },
...
function doActionIBSheet(sheetObj, formObj, sAction, Row, ColName) {
    ...
    switch (sAction) {
        case IBSEARCH: // retrieve
            ...
            var sXml=sheetObj.GetSearchData("FNS_JOO_0201GS.do", param);
        ...
        case IBSAVE: // save
            ...
            var sXml = sheetObj.GetSaveData("FNS_JOO_0201GS.do", param);
		...
		case IBROWSEARCH:    // Row Search
		    ...
		    sheetObj.DoRowSearch(Row, "FNS_JOO_0201GS.do", queryStr, {Sync:2});
		break;
    }
}
...
function sheet1_OnDblClick(sheetObj, Row, Col){
	with(sheetObj){
		...
		switch (colName) {
			case "rev_vvd":
				if(GetCellEditable(Row, Col)){
					var sUrl = "JOO_VVD_LIST.do";
					ComOpenPopup(sUrl, 463, 450, "setCallBackVVD", "0,0", true);
				}
				break;
		}
	}
		...
		with(sheetObj){
			if(rtnDatas){
				if(rtnDatas.length > 0){
					rtvVvd = rtnDatas[1] + rtnDatas[2] + rtnDatas[3];
				}
			}
			SetCellValue(GetSelectRow(), "rev_vvd", rtvVvd, 1);
		}
	}
} 
