    ...
    try {
        ...
        switch (srcName) {
            ...
            case "btn_New":
                ...
                comboObjects[1].SetSelectCode(gCurrencyUSD);
                //comboObjects[1].SetSelectIndex(-1, false);
                comboObjects[2].SetSelectIndex(0);
                comboObjects[3].SetSelectIndex(0);
                comboObjects[4].SetSelectIndex(-1, false);
                comboObjects[5].SetSelectCode(gCurrencyKRW);
...
function initCombo(comboObj, comboNo) {
    ...
    switch (comboObj.options.id) {
        ...
        case "inv_curr_cd": 
            ...
            var comboItems = gCurrencyCdComboItems.split(ROWMARK);
            addComboItem(comboObj, comboItems);
            comboObj.SetSelectCode(gCurrencyKRW);
            break;
        case "jo_stl_itm_cds":
        	with (comboObj) {
        		SetMultiSelect(true);
...
function initControl() {
    ...
    */ 
    ...
    doActionIBSheet(sheetObjects[0], document.form, IBSEARCH_ASYNC03); //local Exchange rate 조회.
    ...
    switch (sheetObj.id) {
        case "sheet1": // t1sheet1 init     //t1sheet1 init
            with(sheetObj){
                ...
                var HeadTitle1="|Partner|Lane|Curr.|Revenue|Expense|Local Invoice|Local Invoice|Local Invoice|Local Invoice|Invoice\nTarget|Partner|Partner|acctg_crr_cd|Invoice Month|dtl_grp_key|cre_inv_grp_key|rqst_dt|inv_iss_dt";
                var HeadTitle2="|Partner|Lane|Curr.|Revenue|Expense|Curr.|Ex.Rate|Revenue|Expense|Invoice\nTarget|Customer|S. Provider|acctg_crr_cd|Invoice Month|dtl_grp_key|cre_inv_grp_key|rqst_dt|inv_iss_dt";
                ...
                var cols = [ 
                       ...
                       {Type:"Text",      Hidden:0, Width:50,   Align:"Center",  ColMerge:1,   SaveName: prefix + "inv_curr_cd",    KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
                       {Type:"Float",     Hidden:0, Width:80,   Align:"Center",  ColMerge:1,   SaveName: prefix + "locl_xch_rt",    KeyField:0,   CalcLogic:"",   Format:"NullFloat",   UpdateEdit:0,   InsertEdit:0 },
                       {Type:"Float",     Hidden:0, Width:100,  Align:"Right",   ColMerge:0,   SaveName: prefix + "stl_rev_locl_amt",KeyField:0,   CalcLogic:"",   Format:"NullFloat",   PointCount:2,   UpdateEdit:0,   InsertEdit:0 },
                       {Type:"Float",     Hidden:0, Width:100,  Align:"Right",   ColMerge:0,   SaveName: prefix + "stl_exp_locl_amt",KeyField:0,   CalcLogic:"",   Format:"NullFloat",   PointCount:2,   UpdateEdit:0,   InsertEdit:0 },
        ...
        case "sheet2": // t1sheet1 init     //t1sheet1 init
            with(sheetObj){
                var HeadTitle1="|Lane|Item|Revenue|Revenue|Revenue|Expense|Expense|Expense|Remark";
                    HeadTitle1+="|VVD|VVD|VVD|Rev.Dir|Rev.Month|STL.Seq.|Partner|BSA|Curr.|Rev/Exp|dtl_grp_key";
                var HeadTitle2="|Lane|Item|VVD|Amount|Locl Amount|VVD|Amount|Locl Amount|Remark";
                ...
                var cols = [ 
                       ...
                       {Type:"Float",     Hidden:0, Width:90,   Align:"Right",   ColMerge:0,   SaveName: prefix + "rev_locl_amt",    KeyField:0,   CalcLogic:"",   Format:"NullFloat",   PointCount:2,   UpdateEdit:0,   InsertEdit:0 },
                       ...
                       {Type:"Float",     Hidden:0, Width:90,   Align:"Right",   ColMerge:0,   SaveName: prefix + "exp_locl_amt",    KeyField:0,   CalcLogic:"",   Format:"NullFloat",   PointCount:2,   UpdateEdit:0,   InsertEdit:0 },
                       {Type:"Text",      Hidden:0, Width:200,   Align:"Left",    ColMerge:0,   SaveName: prefix + "stl_rmk",        KeyField:0,   CalcLogic:"",   Format:"",            PointCount:0,   UpdateEdit:0,   InsertEdit:0 },
...
function doActionIBSheet(sheetObj, formObj, sAction, cRow) {
    ...
    switch (sAction) {
        case IBSEARCH: // retrieve
            ...
            var sXml = sheetObj.GetSearchData("FNS_JOO_0203GS.do", param);
        ...
        case IBSAVE: // save
            ...
            var sXml = sheetObj.GetSaveData("FNS_JOO_0203GS.do", sParam);
        ...
        case IBSEARCH_ASYNC03: //Local Exchange Rate
            formObj.f_cmd.value = SEARCH27;
            
            var invCurrCd = inv_curr_cd.GetSelectCode();
            var param = FormQueryString(formObj);
                param += "&attr_ctnt1=" + invCurrCd; //북수형
            
            var sXml = sheetObj.GetSearchData("JOOCommonGS.do", param);
            //ComXml2ComboItem(sXml, trd_cd, "code", "code");
            
            if (ComGetEtcData(sXml, "TRANS_RESULT_KEY") == "S") {
	            var tmpLoclXchRt      = ComGetEtcData(sXml, "locl_xch_rt");
	            ComSetObjValue(formObj.locl_xch_rt, tmpLoclXchRt);
	            
	            setRefresh();//조건에 해당하는 데이타 재조회.
            }
...
function setShowFooterRow(){
	...
	if (sheetObj.RowCount() > 0) {
		...
		var iRevLoclAmt = sheetObj.ComputeSum('|sheet2_rev_locl_amt|');
		var iExpLoclAmt = sheetObj.ComputeSum('|sheet2_exp_locl_amt|');
	
		//var balanceAmt = Math.abs(JooComGetRound(iRevActAmt - iExpActAmt, 2));
		var balanceLoclAmt = Math.abs(JooComGetRound(iRevLoclAmt - iExpLoclAmt, 2));
		/*
		...
		*/    
		...
		sheetObj.ShowFooterRow([{
			...
			"sheet2_rev_vvd#ColSpan": 3,
			"sheet2_rev_vvd": iRevLoclAmt, 
			...
			"sheet2_exp_vvd": iExpLoclAmt,
	        "sheet2_exp_vvd#ColSpan": 3, 
	        ...
	        "sheet2_rlane_cd#FontBold": 1,
	        "sheet2_rlane_cd#FontColor": "Blue",
			"sheet2_rev_vvd": balanceLoclAmt,
	        "sheet2_rev_vvd#ColSpan": 6, 
	        "sheet2_rev_vvd#Align": "right", 
	        "sheet2_rev_vvd#Type": "Float",
	        "sheet2_rev_vvd#Format": "NullFloat",
	        "sheet2_rev_vvd#PointCount": 2,
...
function validateForm(sheetObj, formObj, sAction) {
    switch (sAction) {
	    case IBSEARCH:
	    	...
	    	if(ComIsEmpty(formObj.inv_curr_cd)){
	    		//JOO00115 Please select  {?msg1}.
	    		ComShowCodeMessage("JOO00115", "Invoice Curr");
	    		return false;
	    	}
	    	if(ComIsEmpty(formObj.locl_xch_rt)){
	    		//JOO00116 Please input  {?msg1}.
	    		ComShowCodeMessage("JOO00116", "Invoice exchange rate");
	    		return false;
	    	}
	    	break;
        case IBSAVE: //save
...
function inv_curr_cd_OnChange(comboObj, oldIndex, oldText, oldCode, newIndex, newText, newCode) {
	...
	initAllSheetRemove(); //조회 된 Sheet데이타 삭제 처리 한다.
...
//  chkField(obj, 'date', false, null, null, '-');
