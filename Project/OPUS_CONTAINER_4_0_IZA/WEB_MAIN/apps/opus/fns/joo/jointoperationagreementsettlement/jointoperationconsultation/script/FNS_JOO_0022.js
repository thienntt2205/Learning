	...
	try {
		...
		switch (srcName) {
			...
			case "btn_csr":
				...
				ComJooCsrDetailPopup(param, true);
				break;
			case "btn_approval":
	...
	switch (sheetNo) {
		case 1: //t1sheet1 init
			with (sheetObj) {	       
		        ...
		        var HeadTitle	="|GRP No|CHK|Approval\n/Cancel|Cancel Reason|Offset No|Register No.|Issue Team|CSR Creation Date|Req. Date|Eff. Date|Issuer|Vendor\n/Customer|Vendor\n/Customer Name|Currency|Amount|Description|Error Message";
		        	...
		        	HeadTitle  +="|clz_sts_cd|auth_cd|lst_apro_flg|apro_rqst_no|apro_rqst_seq|inv_no|apro_flg|apro_dt|cxl_flg|evid_tp_cd|chk_grp_csr_offst_no|csr_offst_flg|cre_usr_id|inv_qty";
...
function initControl() {
	...
	//axon_event.addListenerFormat('beforedeactivate', 'obj_blur' , formObject);
...
//			}
		...
		case IBSAVE: //save
            ...
            var sXml = sheetObj.GetSaveData("FNS_JOO_0022GS.do", sParam);
		...
		case IBSEARCH_ASYNC10: // BackEndJob Status
		    ...
		    var sXml = sheetObj.GetSearchData("FNS_JOO_0022GS.do", "f_cmd=" + SEARCH02 + "&backendjob_key=" + ComGetObjValue(formObj.backendjob_key));
		    var jobState = ComGetEtcData(sXml, "jb_sts_flg");
			 	...
			 	if (jobState == "3") { // Success
			 		...
			 		ComOpenWait(false);
			 		clearInterval(timer);
			 	    var sXml = sheetObj.GetSearchData("FNS_JOO_0022GS.do", "f_cmd=" + SEARCH03 + "&backendjob_key=" + ComGetObjValue(formObj.backendjob_key) + "&csr_offst_no=" + ComGetObjValue(formObj.csr_offst_no));
...
function setGroupCellValue(sheetObj, formObj, val){	
	if (sheetObj.RowCount() > 0) {
        ...
        // Issuer와 Approver 같을시 Approval 불가 - 2018.05.30
        for(idx=0; idx<arrChkRow.length; idx++){
        	if(gUsrId == sheetObj.GetCellValue(arrChkRow[idx], prefix+"cre_usr_id")){
        		ComShowCodeMessage('JOO00226',sheetObj.GetCellValue(arrChkRow[idx], prefix+"issuer"));
        		return false;
        	}
        }
        for(idx=0; idx<arrChkRow.length; idx++){
