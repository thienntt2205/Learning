...
<%
	...
	//testVo = (ComUserVO)event.getComUserVO();
	//strLangType = testVo.getLangTpCd().toLowerCase();
// 	strLangType = ((String)request.getSession().getAttribute(SignOnFilter.FORM_LANG_TP_CD_TYPE)).toLowerCase();
...
var itemComboText2 = "<%=codeinfo2[1]%>";
	...
	function fn_change(){
		...
		//if(document.all("local_lang_type").value.length < 1){
			//ComAlertFocus(document.all("local_lang_type"), "Value Null !");
			//return ComShowMessage("");
		//}
		var langType = frm.scr_msg_code.value;
		//alert("document.all(local_lang_type).value :"+document.all("local_lang_type").value);
		ComShowMessage(ComGetMsg(langType));
		//frm.action = "ScriptMultyLangAccess.do";
		//frm.submit();
	}
...
<table style="margin-left: 20px;" align="center">
	<tr>
		<td align="left"><fmt:message key="lang.table.title"></fmt:message></td>
		<td> : </td>
		<td align="left">
			<select name="local_lang_type"  style="width: 60px;" onchange="javascript:fn_langChange();">
	...
	<tr>
		<td align="left"><fmt:message key="script.lang.table.colum1"></fmt:message></td>
		<td> : </td>
		<td align="left">
			<select name="scr_msg_code" style="width: 150px;" onchange="javascript:fn_change();">
				<option value="">-- 선 택 --</option>
				<option value="COM12111">COM12111</option>
				<option value="COM12112">COM12112</option>
				<option value="CCD00001">CCD00001</option>
			</select>
	</tr>
</table>
